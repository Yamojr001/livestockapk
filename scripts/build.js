...
function getDeploymentDomain() {
  // Use EXPO_PUBLIC_DOMAIN or command line argument
  if (process.env.EXPO_PUBLIC_DOMAIN) {
    return stripProtocol(process.env.EXPO_PUBLIC_DOMAIN);
  }

  // Check for domain as command line argument
  const args = process.argv.slice(2);
  if (args.length > 0) {
    return stripProtocol(args[0]);
  }

  console.error(
    "ERROR: No deployment domain found. Set EXPO_PUBLIC_DOMAIN environment variable or provide domain as command line argument.",
  );
  process.exit(1);
}
...
function clearMetroCache() {
  ...
  // Define possible cache directories
  ...
  for (const dir of cacheDirs) {
    try {
      ...
      console.warn(`Warning: Could not clear ${dir}: ${error.message}`);
    }
  }
...
async function main() {
  ...
  console.log("Manifests available at:");
  console.log(`  - ${baseUrl}/ios/manifest.json`);
  console.log(`  - ${baseUrl}/android/manifest.json`);
